...
function selectRandomQuote() {
  let randomVal = Math.floor(Math.random() * quotes.length);
  let getRandomQuote = quotes[randomVal];

  quote.innerText = `${getRandomQuote.quote}`;
  source.innerText = `${getRandomQuote.source}`;

  let hexCode = "ABCDEF1234567890";
  let color = "#";

  for (let i = 0; i < 6; i++) {
    let hexRandomVal = Math.floor(Math.random() * 16 + 1);
    color += hexCode[hexRandomVal];
  }
